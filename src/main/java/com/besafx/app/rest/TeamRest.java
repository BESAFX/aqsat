...
public class TeamRest {
    ...
    public String create(@RequestBody Team team, Principal principal) {
        ...
        notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم انشاء مجموعة صلاحيات جديدة بنجاح" : "Create Privileges Successfully").type("success").build(), principal.getName());
    ...
    public String update(@RequestBody Team team, Principal principal) {
        ...
        if (object != null) {
            ...
            Person caller = personService.findByEmail(principal.getName());
            String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم تعديل الصلاحيات بنجاح" : "Update Privileges Successfully").type("warning").build(), principal.getName());
            return SquigglyUtils.stringify(Squiggly.init(new ObjectMapper(), FILTER_TABLE), team);
        } else {
            return null;
        }
    ...
    public void delete(@PathVariable Long id, Principal principal) {
        ...
        if (team != null) {
            ...
            Person caller = personService.findByEmail(principal.getName());
            String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم حذف مجموعة الصلاحيات بنجاح" : "Delete Privileges Successfully").type("error").build(), principal.getName());
        }
    }
