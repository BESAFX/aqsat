...
public class EmployeeRest {
    ...
    public String create(@RequestBody Employee employee, Principal principal) {
        ...
        String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم انشاء حساب الموظف بنجاح" : "Create Employee Account Successfully").type("success").build(), principal.getName());
    ...
    public String update(@RequestBody Employee employee, Principal principal) {
        ...
        if (object != null) {
            ...
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم تعديل بيانات حساب الموظف بنجاح" : "Update Employee Account Information Successfully").type("warning").build(), principal.getName());
            return SquigglyUtils.stringify(Squiggly.init(new ObjectMapper(), FILTER_TABLE), employee);
        } else {
            return null;
        }
    ...
    public void delete(@PathVariable Long id, Principal principal) {
        ...
        if (employee != null) {
            ...
            Person caller = personService.findByEmail(principal.getName());
            String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم حذف حساب الموظف بنجاح" : "Delete Employee Account Successfully").type("error").build(), principal.getName());
        }
    }
