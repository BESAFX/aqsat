...
public class ReceiptRest {
    ...
    public String create(@RequestBody Receipt receipt, Principal principal) {
        ...
        notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم انشاء السند بنجاح" : "Create Receipt Account Successfully").type("success").build(), principal.getName());
    ...
    public String update(@RequestBody Receipt receipt, Principal principal) {
        ...
        if (object != null) {
            ...
            Person caller = personService.findByEmail(principal.getName());
            String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم تعديل بيانات السند بنجاح" : "Update Receipt Account Information Successfully").type("warning").build(), principal.getName());
            return SquigglyUtils.stringify(Squiggly.init(new ObjectMapper(), FILTER_TABLE), receipt);
        } else {
            return null;
        }
    ...
    public void delete(@PathVariable Long id, Principal principal) {
        ...
        if (receipt != null) {
            ...
            Person caller = personService.findByEmail(principal.getName());
            String lang = JSONConverter.toObject(caller.getOptions(), Options.class).getLang();
            notificationService.notifyOne(Notification.builder().message(lang.equals("AR") ? "تم حذف السند وكل ما يتعلق به من حسابات بنجاح" : "Delete Receipt Account With All Related Successfully").type("error").build(), principal.getName());
        }
    }
